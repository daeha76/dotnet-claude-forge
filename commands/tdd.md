...
> **스택 감지**: 프로젝트에 `*.slnx` / `*.sln` / `*.csproj` 가 있으면 .NET(xUnit) 방식으로 진행한다. `package.json`이 있으면 JS/TS(Jest/Vitest) 방식으로 진행한다.
...
Error Message: System.NotImplementedException
```
...
- .NET: `WebApplicationFactory<Program>` 기반 통합 테스트
- .NET: Testcontainers로 PostgreSQL/Supabase 격리 환경 구성
